...
public final class MatchQueue {
    ...
    private void createMatchAndRemoveEntries(MatchQueueEntry entryA, MatchQueueEntry entryB) {
        ...
        if (match != null) {
            ...
            teamA.messageAlive(foundStart + Joiner.on(", ").join(PlayerUtils.mapToNames(teamB.getAllMembers())) + teamBElo);
            teamB.messageAlive(foundStart + Joiner.on(", ").join(PlayerUtils.mapToNames(teamA.getAllMembers())) + teamAElo);
