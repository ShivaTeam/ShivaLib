...
import team.shiva.shivalib.scoreboard.ScoreFunction;
 ...
 * </ul>
 */
final class MatchScoreGetter implements BiConsumer<Player, ArrayList<String>> {
    ...
    private boolean renderParticipantLines(List<String> scores, Match match, Player player) {
        ...
        return match.getState().equals(MatchState.IN_PROGRESS) &&
                PotPvPSI.getInstance().getSettingHandler().getSetting(player, Setting.SHOW_PING_ON_SCOREBOARD);
    }

    private void render1v1MatchLines(List<String> scores, MatchTeam otherTeam) {
        scores.add("&c&lOpponent: &f" + UUIDUtils.name(otherTeam.getFirstMember()));
