...
public final class MongoEloRepository implements EloRepository {
    ...
    @CommandMeta(label = { "recalcGlobalElo" }, permission = "op")
    public static class RecalcGlobalElo{
        public void execute(Player sender){
            recalcGlobalElo(sender);
        }
    }
        ...
        try {
            MongoUtils.getCollection(MONGO_COLLECTION_NAME).find().sort(Sorts.descending(kitTypeName)).limit(10).forEach(new Consumer<Document>() {
                ...
                public void accept(Document document) {
                    ...
                    toInsert.put(PlayerUtils.getFormattedName(UUID.fromString((String) document.get("players", ArrayList.class).get(0))), elo);
                }
            });
