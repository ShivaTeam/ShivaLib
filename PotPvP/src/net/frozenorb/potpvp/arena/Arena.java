...
import team.shiva.shivalib.cuboid.Cuboid;
...
public final class Arena {
    ...
    private transient final Set<BlockVector> changedBlocks = new HashSet<>();
    ...
    public void recordBlockChange(Block block) {
        changedBlocks.add(block.getLocation().toVector().toBlockVector());
    }
    public void restore() {
        ArenaHandler arenaHandler = PotPvPSI.getInstance().getArenaHandler();
        World world = arenaHandler.getArenaWorld();
        ArenaSchematic arenaSchematic = arenaHandler.getSchematic(schematic);
        Vector weVector = arenaSchematic.getModelArenaLocation();
        int offsetX = weVector.getBlockX() - bounds.getLowerX();
        int offsetY = weVector.getBlockY() - bounds.getLowerY();
        int offsetZ = weVector.getBlockZ() - bounds.getLowerZ();
        for(BlockVector vector: changedBlocks){
            Block current = world.getBlockAt(vector.toLocation(world));
            Block origin = world.getBlockAt(vector.toLocation(world).add(offsetX, offsetY, offsetZ));
            current.setType(origin.getType());
            current.setData(origin.getData());
        }
    }
}